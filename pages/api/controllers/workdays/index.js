...
const { basic } = require("../../services/workdays");
...
const {
  ...
  create: createWorkdaysSchema,
  update: updateEmployeeSchema,
  remove: deleteEmployeeSchema,
} = require("../../validations/workdays");
...
export default requireAuth(async (req, res) => {
  ...
  switch (method) {
    ...
    default:
      ...
      result = response(UNPROCESSABLE_ENTITY, `Method ${method} Not Allowed`);
  }
...
const create = async (payload) => {
  try {
    const { error } = createWorkdaysSchema.body.validate(payload);
    if (error) {
      const { details } = error;
