...
const { basic } = require('../../services/crons');
const requireAuth = require('../../middlewares/_requireAuth')

const {
    getById: getCronSchema,
    create: createCronSchema,
    update: updateCronSchemsa,
    delete: deleteCronSchema,
   // list: listCronSchema
} = require('../../validations/crons');
...
export const getById = async (id) => {
    try {
        const { error } = getCronSchema.query.validate({ id });
        if (error) {
            const { details } = error;
...
export const create = async (payload) => {
    try {
        const { error } = createCronSchema.body.validate(payload);
        if (error) {
            const { details } = error;
            const message = details.map(i => i.message.replace(/\"/g, '')).join(',');
            return response(UNPROCESSABLE_ENTITY, message);
...
export const update = async (payload, id) => {
    try {
        ...
        const { error } = updateCronSchemsa.body.validate(payload);
        if (error) {
            const { details } = error;
            const message = details.map(i => i.message.replace(/\"/g, '')).join(',');
            return response(UNPROCESSABLE_ENTITY, message);
...
export const remove = async (id) => {
    try {
        ...
        const { error } = deleteCronSchema.query.validate({ id });
        if (error) {
            const { details } = error;
            const message = details.map(i => i.message.replace(/\"/g, '')).join(',');
            return response(UNPROCESSABLE_ENTITY, message);
